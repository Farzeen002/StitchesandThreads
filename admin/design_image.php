				...
				<form  action="" method="POST" enctype="multipart/form-data">
					...
					<div class="col-md-3"></div>
						...
						<div class="form-group clearfix">
							<button type="submit" class="theme-btn btn-small" name="submit">DESIGN ADDED</button>
							</div>
						</div>
						<div class="col-md-3"></div>
					</fieldset>
...
if(isset($_POST['submit']))
{
    ...
    if($imgtype != "image/jpg" && $imgtype != "image/png" && $imgtype != "image/jpeg" ) 
    {
        echo "<script>alert('File type is not compatible');
                                   window.location='design_image.php';</script>";
    }
    else 
    {
        ...
        {                      
			$Dname=$_POST['name'];
			$qry=mysqli_query($con,"INSERT INTO `tbl_design_data`(`design_id`,`design_name`, `design_image`) VALUES ('','$Dname','$final_file')")or die(mysqli_error($con));
		    if($qry)
		                            {
