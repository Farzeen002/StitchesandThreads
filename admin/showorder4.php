...
$i=1;
$query=mysqli_query($con,"SELECT d.*,m.m_id,m.m_type,u.uid,u.full_name,u.contact,u.gender,u.address,u.email,u.password,u.image as img from `tbl_gown` d inner join tbl_material m on m.m_id=d.m_id  inner join tbl_user u on u.uid=d.uid group by g_id ") or die(mysqli_error($con));
...
<tr>
	...
	<td> <a href="sendsms4.php?cid=<?php echo $row['uid']; ?>&&amt=<?php echo $row['materialprice']; ?> " class="btn btn-warning">Taken</a></td>
			 				<td> 
			 	<?php $id=$row['g_id'];
			 	$sel=mysqli_query($con,"SELECT * FROM tbl_assign where id='$id' and type='gown'");
			 	$read=mysqli_num_rows($sel);
			 	if($read <= 0)
			 	 {
			 		?>
			 <a href="assign.php?id=<?php echo $row['g_id']; ?>&&type=gown&&usr=<?php echo $row['uid'];?>" class="btn btn-info">Assign</a>
