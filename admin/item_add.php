				...
				<form  action="" method="POST" enctype="multipart/form-data">
					...
					<div class="col-md-3"></div>
						...
						<div class=" col-md-6 col-sm-6 col-xs-6 col-offset-sm-3 reg-customers">
							...
							<input type="text" class="form-control" name="iname" placeholder="Enter item name"> <br>
						</div>


						<div class="form-group">
							<label>Item Description:</label><br>
							<textarea name="Des" rows=4 placeholder="Enter item discription"></textarea><br>
						</div>
						...
						<div class="form-group clearfix">
								...
								<button type="submit" class="theme-btn btn-small" name="submit">ADD ITEM</button>
							</div>
						</div>
                        ...
                        if(isset($_POST['submit']))
                        {
                            ...
                            if($imgtype != "image/jpg" && $imgtype != "image/png" && $imgtype != "image/jpeg" ) 
                            {
                                  echo "<script>alert('File type is not compatible');
                                   window.location='item_add.php';</script>";
                            }
                            else {
                            if(move_uploaded_file($file_loc,$folder.$final_file))
                               {                      
						            ...
						            $Price=$_POST['price'];
						            $Qty=$_POST['qty'];
		                            $qry=mysqli_query($con,"INSERT INTO `tbl_item`(`item_id`, `cid`,`item_name`, `item_image`, `item_description`, `item_price`, `item_qty`) VALUES ('','$Ctype','$Name','$final_file','$Descrip','$Price','$Qty')")or die(mysqli_error($con));
		                            if($qry)
		                            {
		                              echo '<script>alert("information added succefully");window.location="view_item.php";</script>';
		                            }
		                           else
		                           {           
