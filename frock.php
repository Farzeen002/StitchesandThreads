		...
		<div id="main" class="haslayout padding-section" style="padding:0px;">
			<div class="container">
				<form class="login-form creataccount-form" action="" method="POST" enctype="multipart/form-data">
					<fieldset>
						<div class="border-left"><h2>FROCK MEASURMENT</h2></div>
						<div class="row">
							...
							<div class="form-group col-sm-6 col-xs-12">
								<label>Bodice:</label>
								<input type="text" class="form-control" name="bodice" placeholder="Bodice in cm" required="" autocomplete="off"><br>
							</div>
							<div class="form-group col-sm-6 col-xs-12">
								<label>Skirt Length:</label>
								<input type="text" class="form-control" name="sklength" placeholder="Shirt length in cm" required="" autocomplete="off"><br>
							</div>
								<div class="form-group col-sm-6 col-xs-12">
								<label>Length:</label>
							...
							<div class="form-group col-sm-6 col-xs-12">
								<label>Sweep:</label>
								<input type="text" class="form-control" name="sweep" placeholder="Sweep in cm" required="" autocomplete="off"><br>
							</div>
							<div class="form-group col-sm-6 col-xs-12">
...
if(isset($_POST['submit']))
    {
        ...
        if(!empty($insertValuesSQL)){
            ...
            // $insert = mysqli_query($con,"INSERT INTO `tbl_frock`(`f_id`, `m_id`, `qty`, `colour`, `cust_id`, `length`, `waist`,  `comment`, `book_date`, `delivery_date`,  `SL`, `Shoulder`, `chest`, `Swidth`, `Bodice`, `SKlength`, `sweep`, `front_image` ) VALUES ('','$Mtype','$Qty','$colour','$user','$length','$waist','$cmt','$Bookdate','$Deliverydate','$slength','$shoulder','$chest','$swidth','$bodice','$sklength','$sweep','$insertValuesSQL')") or die(mysqli_error($con));
             $insert = mysqli_query($con,"INSERT INTO `tbl_frock`(`f_id`, `m_id`, `qty`, `book_date`, `delivery_date`, `colour`, `SL`, `SW`, `chest`, `waist`, `hip`, `bodice`, `SKlength`, `length`, `sweep`, `shoulder`, `comment`, `image`, `uid`,`materialprice`) VALUES ('','$Mtype','$Qty','$Bookdate','$Deliverydate','$colour','$slength','$swidth','$chest','$waist','$Hip','$bodice','$sklength','$length','$sweep','$shoulder','$cmt','$insertValuesSQL','$user','$mprice')") or die(mysqli_error($con));
            ...
            if($insert){
              ...
              echo '<script> alert("Details are uploaded successfully.");window.location="view_frock.php?last_id='.$last_id.'";</script>';
            }
            else
            {
              echo '<script> alert("Sorry, there was an error uploading your details.");window.location="frock.php";</script>';
            }
            }
